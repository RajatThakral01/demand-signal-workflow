...
**Commit:** `HEAD` as of `2026-08-27` (post `S0039` README split to docs; `S0038` validator + `S0036` Groq + `S0037` sweep; `f96b4f9` baseline, now `249 passed` + docs) → will be tagged `project02-submission-v1`
**Date:** 2026-08-27

This is the "Before you send" confirmation per the assessment brief, verified start-to-finish from a genuinely clean environment (fresh clone + `cp .env.example .env` + no manual steps). Updated after the live multi-provider sweep (Groq `openai/gpt-oss-20b` now *recommended* — see `docs/Cost_and_Limits.md` and `docs/Plan_vs_Built.md` — Live multi-provider classification testing) and after `S0038` fresh-clone int-var fix.

## Clean-environment proof (updated)

* **Fresh clone:** `git clone file://$(pwd) /tmp/dsw-clean` from `HEAD` → `cp .env.example .env` → add `ADMIN_API_KEY=test_admin_key` (dummy `OPENROUTER_API_KEY`/`GROQ_API_KEY` for mocked suite; live Groq run used real `GROQ_API_KEY` from `console.groq.com` separately). `.env.example` now ships `RETRY_MAX_ATTEMPTS=3` `RETRY_BASE_DELAY_MS=500` `INTERPRET_MIN_TOKENS=2` (was empty) + `app/config.py:61` `field_validator` treats `""` as missing → `Settings()` no longer `ValidationError int_parsing` on fresh `cp .env.example .env` (see `S0038`).
* **DB:** `DROP DATABASE IF EXISTS dsw_test; CREATE DATABASE dsw_test` (host) or `docker compose down -v` + `docker compose build` (bakes `0013_dlq_unresolved_unique` — was `0012` in earlier checklist) + `docker compose up -d --wait` — verified `Running upgrade 0001 → 0013_dlq_unresolved_unique` + `Uvicorn running` + `curl /health 200 {status:ok,db:ok}`
* **Full suite:** `pytest -q` **249 passed, 1 skipped in ~29–34s** (host, `live` gated) — well under `<300s` (`real` via `scripts/clean_run.sh` still `~28s`). Before this docs-only sweep: `242 passed` (after `S0035` edit-500 fix), `241` (after `0013` varchar fix), `238` (after 10-fix sweep); now `+7` from `S0036` Groq provider unit tests (`test_llm_provider.py:7`) — `S0038` validator is safety-net, count unchanged.
* **Docker config:** `docker compose config` OK (`LLM_PROVIDER openrouter` `GROQ_API_KEY ""` `RETRY_MAX_ATTEMPTS 3` + `TEST_DATABASE_URL` isolated), `docker compose build` OK (now includes `python-multipart` dep for `pages.py:Form`), `health` `{"status":"ok","db":"ok"}` `200`
...
|---|---|---|
| Repo / commit ID | ✅ | `HEAD` as of `2026-08-27` (post `S0039` README split to docs; `S0038` validator + `S0036` Groq + `S0037` sweep) / `git tag project02-submission-v1` → `HEAD` (force-pushed) |
| README | ✅ | `README.md` 232 lines, includes purpose, arch, setup, env vars (now `GROQ_API_KEY`/`LLM_PROVIDER` + `openai/gpt-oss-20b` recommended), usage, troubleshooting + linked docs: `docs/Cost_and_Limits.md` **Cost & Limits with 4 live models** (DeepSeek `212` `$0.000026` historical, Nemotron `>30s` timeout, Gemma `429` `20/min`, Groq `28/28` `16.61s` `0` dead-letters `8/3/4` + `30 RPM / 1k RPD / 8k TPM / 200k TPD` stated), `docs/Plan_vs_Built.md` + **live multi-provider subsection**, `docs/Known_Limitations.md` 8 bullets |
| Architecture diagram | ✅ | `README.md:41` Mermaid with 6 subgraphs `Inputs/Decisions/Agents/Storage/Actions/Failure Paths` |
| Sample config | ✅ | `.env.example` (names only, defaults `LLM_PROVIDER=openrouter` `RETRY_MAX_ATTEMPTS=3` `RETRY_BASE_DELAY_MS=500` `INTERPRET_MIN_TOKENS=2` + `GROQ_API_KEY=` `OPENROUTER_API_KEY=` with `console.groq.com` comment, per convention) + `docker-compose.yml:35` `GROQ_API_KEY`/`LLM_PROVIDER` with `${VAR:-...}` + `app/config.py:61` `field_validator` empty-string safety net |
...
| .env.example | ✅ | `.env.example` (`LLM_PROVIDER`/`GROQ_API_KEY` + `RETRY_MAX_ATTEMPTS=3`/`RETRY_BASE_DELAY_MS=500`/`INTERPRET_MIN_TOKENS=2` — was empty, now valid default; validator remains safety net) |
| Test report | ✅ | `pytest -q` **249 passed, 1 skipped** (was `221` → `+7` Groq provider `test_llm_provider.py` + `+1` edit-500 fix + `+2` migration + `+18` earlier; `S0038` validator unchanged count); `docs/evidence/pytest_final.txt` pasted real `249 passed` output as of this task |
...
| Known limitations | ✅ | `docs/Known_Limitations.md` 8 bullets (single-process, SIMULATED only, no tenant/RBAC, no retention, evaluator tooling only, on-read escalated, single-model cost, host socket) |
| Plan-vs-built summary | ✅ | `docs/Plan_vs_Built.md` (OpenRouter `deepseek` switch + **Live multi-provider classification testing** subsection: why free-tier re-test, 4 models in order, final `openai/gpt-oss-20b` via Groq, 2 defects `S0034`/`S0035` as evidence live testing matters) + `fuzzy always manual`, `primary_company`, `UNIQUE`s, `escalated` on-read, `partial UNIQUE`, dashboard |
| AI_USAGE.md | ✅ | `AI_USAGE.md` **38 sessions** `S0001–S0039` (with `S0021` gap documented) incremental, each `Provider/model/What was generated/Human review/Verification` at phase end; last `S0039` is README split to `docs/Cost_and_Limits.md`/`docs/Plan_vs_Built.md`/`docs/Known_Limitations.md` (Muse Spark `muse-spark-1.2`, `307→232` lines) |
| ai-usage.json | ✅ | `ai-usage.json` **38 sessions** `S0001–S0039` (with `S0021` skipped, documented in top-level `notes`), every `evaluation_id/project/session_id/started_at/ended_at/provider/model/version/prompts_or_hashes/export_path/skills_tools/generated_files/human_modified_files/verification/costs/notes` populated (validator PASS), last `S0039` mirrors `AI_USAGE.md` |
| Asset provenance | ✅ | `README.md:211` `## Asset Provenance: No external icon/font/dataset beyond fixtures` + `fixtures/* @example.com` + `style.css` hand-written + `difflib` only |
| Cost/rate-limit statement | ✅ | `docs/Cost_and_Limits.md` **full rewrite** — 4 live models: DeepSeek `212` `$0.000026` (key now expired, historical), Nemotron `>30s` `timeout` at `10s`/`30s`, Gemma `429` `free-models-per-min` `limit 20`, Groq `openai/gpt-oss-20b` `28/28` `16.61s` `0` dead-letters `8/3/4` + `30 RPM / 1k RPD / 8k TPM / 200k TPD` (Groq stated, not independently verified beyond not hitting it), **recommended `openai/gpt-oss-20b` via Groq** (only one to complete full clean run), `CLASSIFICATION_MODEL` table row now `openai/gpt-oss-20b` with OpenRouter alternative still supported. No estimate presented as measurement; Groq per-call token data noted as not available after DB reset. |
| Evidence export | ✅ | `docs/evidence/` — `reconciliation_live_groq_run.json` (pasted `variance=0` `PASS` 8 rows), `logs_sample_redacted.jsonl` (4 lines, `sha256:` PII, 7 fields), `pytest_final.txt` (`249 passed, 1 skipped` as of this task, re-captured after `S0038`) — all re-grepped for secrets (see Secret / PII grep above, now also covering `docs/evidence/`, still clean) |
...
`project02-submission-v1` → `HEAD` as of `2026-08-27` post `S0039` (moved from `b63caa3` → `4c36db5` → current `HEAD`; force-pushed)
